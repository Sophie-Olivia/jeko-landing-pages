...
- [x] **Step 6 — First real page (`pages/lingerie/v1/`).** 15-section advertorial for the Sophie & Olivia Joyce Set following [REFERENCE.md](./REFERENCE.md). 887 lines, mobile-first, 10+ CTAs including a sticky mobile bottom CTA, all to `/lingerie/v1/cta`. Inline JS appends current page UTMs to all CTA hrefs before any click so UTMs survive the `/cta → Shopify` hop. Named competitor reviews (Victoria's Secret, Skims, Lounge, Adore Me) in Phase 1/2 cards, comparison table, and final ranking. Competitor images served from `/public/` (the two PNGs are ~6MB each — see "Image optimization" task in this README).
